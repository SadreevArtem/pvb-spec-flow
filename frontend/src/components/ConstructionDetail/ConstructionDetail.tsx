...
export const ConstructionDetail: React.FC<Props> = ({ id }) => {
  const isEdit = id !== 0;
  const token = useAuthStore((state) => state.token);
  const t = useTranslations("DirectoryDetail");
  const queryClient = useQueryClient();

  const getConstructionById = () => api.getConstructionByIdRequest(id, token);
  const getQueryKey = (id: number) => ["constructions"].concat(id.toString());
  const router = useRouter();

  const { data: construction, isLoading } = useQuery<ConstructionType>({
    queryKey: getQueryKey(id),
    queryFn: getConstructionById,
    enabled: id !== 0,
  });
  const {
    register,
  ...
  const updateConstructionFunc = (input: Inputs) =>
    api.updateConstructionRequest(input, token);
  const createConstructionFunc = (input: Inputs) =>
    api.createConstructionRequest(input, token);
  const deleteFunc = () => api.deleteConstructionRequest(id, token);

  const { mutate: mutation, isPending } = useMutation({
    mutationFn: isEdit ? updateConstructionFunc : createConstructionFunc,
    onSuccess: () => {
  ...
  useEffect(() => {
    ...
    Object.keys(construction).forEach((key) => {
      if (key in construction) {
        setValue(
          key as keyof ConstructionType,
          construction[key as keyof ConstructionType] as string
        );
      }
    });
